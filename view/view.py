...
from utils.message import mostrar_tutorial
from lib.class_electron import plot
from controller.control import (mostrar_radial,mostrar_imag_esfericos,mostrar_real_esfericos,
...
def GUI():
    ...
    # Botones principales
    tk.Button(root, text="Calcular Radial", command=on_radial, bg="#4682B4", fg="white").grid(row=2, column=0, pady=10)
    tk.Button(root, text="Función 2D", command=on_wf_2D, bg="#32CD32", fg="white").grid(row=2, column=1, pady=10)
    tk.Button(root, text="Función 3D", command=on_wf_3D, bg="#FFA500", fg="white").grid(row=2, column=2, pady=10)
    tk.Button(root, text="Sph. Real", command=on_real_esfericos, bg="#4B0082", fg="white").grid(row=8, column=0, pady=5)
    tk.Button(root, text="Sph. Imag.", command=on_imag, bg="#483D8B", fg="white").grid(row=8, column=1, pady=5)
    tk.Button(root, text="sp", command=lambda: on_hibridaciones(p.plot_sp), bg="#708090", fg="white").grid(row=9, column=0, pady=5)
    tk.Button(root, text="sp2", command=lambda:on_hibridaciones(p.plot_sp2), bg="#708090", fg="white").grid(row=9, column=1, pady=5)
    tk.Button(root, text="sp3", command=lambda: on_hibridaciones(p.plot_sp3), bg="#708090", fg="white").grid(row=9, column=2, pady=5)
    tk.Button(root, text="sp2d", command=lambda: on_hibridaciones(p.plot_sp2d), bg="#2F4F4F", fg="white").grid(row=10, column=0, pady=5)
    tk.Button(root, text="sp3d", command=lambda: on_hibridaciones(p.plot_sp3d), bg="#2F4F4F", fg="white").grid(row=10, column=1, pady=5)
    tk.Button(root, text="sp3d2", command=lambda: on_hibridaciones(p.plot_sp3d2), bg="#2F4F4F", fg="white").grid(row=10, column=2, pady=5)
